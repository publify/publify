class Category < ActiveRecord::Base
  ...
  def self.find_all_with_article_counters
    self.find_by_sql([%{
      ...
      ORDER BY position}, true]).each {|item| item.article_counter = item.article_counter.to_i }
  end

  def self.find_by_permalink(permalink, options = {})
  ...
  def permalink_url(anchor=nil, only_path=false)
    ...
    blog.url_for(controller: '/categories', action: 'show', id: permalink, only_path: only_path)
  end
