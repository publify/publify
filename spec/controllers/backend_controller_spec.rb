...
describe BackendController do
  ...
  describe "when called through Blogger API" do
    ...
    it "test_blogger_new_post" do
      alice = create(:user, :login => 'alice', :password => 'whatever')
  ...
  describe "when called through the MetaWeblog API" do
    ...
    it "test_meta_weblog_get_recent_posts" do
      article = create(:article, :created_at => Time.now - 1.day, :allow_pings => true, :published => true)
      article_before = create(:article, :created_at => Time.now - 2.day, :allow_pings => true, :published => true)
      create(:trackback, :article => article, :published_at => Time.now - 1.day, :published => true)
      create(:trackback, :article => article_before, :published_at => Time.now - 3.day, :published => true)
    ...
    it "test_meta_weblog_new_post_fails" do
      @article = Article.new(:title => 'test', :body => 'body', :extended => 'extended', :text_filter => TextFilter.find_by_name('textile'), :published_at => Time.now.utc.midnight)
      ...
      lambda { invoke_layered :metaWeblog , :newPost, *args }.should \
        raise_error(XMLRPC::FaultException, 'Internal server error (exception raised)')
    end
