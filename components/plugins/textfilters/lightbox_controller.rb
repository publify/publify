...
class Plugins::Textfilters::LightboxController < TextFilterPlugin::MacroPost
  KEY='84f652422f05b96b29b9a960e0081c50'
  plugin_display_name "Lightbox"
  plugin_description "Automatically generate tags for Flickr images displayed in a lightbox"

  def self.help_text
    %{
You can use `<typo:lightbox>` to display images from [Flickr](http://flickr.com)
which, when clicked, will be shown in a lightbox using Lokesh Dhakar's [Lightbox](http://www.huddletogether.com/projects/lightbox/) Javascript

Example:
      
    <typo:lightbox img="31367273" thumbsize="thumbnail" displaysize="original"/>
          
will produce an `<img>` tag showing image number 31367273 from Flickr using the thumbnail image size.  The image will be linked to the original image file from Flickr.  When the link is clicked, the larger picture will be overlaid
...
* **alt** The alt text associated with the image.  By default, this is the same as the title.
  ...
  def self.macrofilter(controller,content,attrib,params,text="")
    ...
    %{<div style="#{style}" class="lightboxplugin"><a href="#{displayurl}" rel="lightbox" title="#{title}"><img src="#{thumburl}" width="#{width}" height="#{height}" alt="#{alt}" title="#{title}"/></a>#{captioncode}</div>}
  end
